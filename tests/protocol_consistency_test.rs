use aka_lib::{get_config_path, DaemonRequest, DaemonResponse, ProcessingMode, AKA};
use std::fs;
...
"#;
    ...
    let test_cases = vec![("test_alias", true), ("test_alias", false)];
    ...
    for (cmdline, eol) in test_cases {
        ...
        let mut aka_direct =
            AKA::new(eol, home_dir.clone(), config_path.clone()).expect("Failed to create AKA for direct mode");
        let direct_result = aka_direct
            .replace_with_mode(cmdline, ProcessingMode::Direct)
        ...
        let daemon_result = aka_daemon
            .replace_with_mode(cmdline, ProcessingMode::Daemon)
            .expect("Daemon mode should work");

        // Results should be identical
        assert_eq!(
            direct_result, daemon_result,
            "Direct and daemon modes should produce identical results for eol={}",
            eol
        );
    }
}
...
fn test_query_request_has_eol() {
    ...
    assert!(
        serialized_false.contains("eol"),
        "Serialized query should contain eol field"
    );
    assert!(
        serialized_false.contains("false"),
        "Serialized query should contain eol value"
    );
}
...
fn test_protocol_message_tags() {
    ...
    assert!(
        ...
        "Health request should have correct type tag"
    );
    ...
    let serialized = serde_json::to_string(&query).expect("Should serialize");
    assert!(
        serialized.contains("\"type\":\"Query\""),
        "Query request should have correct type tag"
    );
    ...
    assert!(
        ...
        "Success response should have correct type tag"
    );
    ...
    assert!(
        ...
        "Error response should have correct type tag"
    );
}
...
fn test_request_type_differentiation() {
    let requests_json = vec![
        r#"{"type":"Health"}"#,
        r#"{"type":"Query","version":"v0.5.0","cmdline":"test","eol":true}"#,
        r#"{"type":"List","version":"v0.5.0","global":false,"patterns":[]}"#,
        r#"{"type":"Freq","version":"v0.5.0","all":false}"#,
        r#"{"type":"ReloadConfig"}"#,
        r#"{"type":"Shutdown"}"#,
    ];
