...
use aka_lib::{determine_socket_path, get_config_path, DaemonRequest, DaemonResponse, ProcessingMode, AKA};
...
fn test_daemon_protocol_serialization() {
    ...
    for request in requests {
        ...
        let deserialized: DaemonRequest = serde_json::from_str(&serialized).expect("Request should deserialize");
...
fn test_daemon_response_serialization() {
    ...
    for response in responses {
        ...
        let deserialized: DaemonResponse = serde_json::from_str(&serialized).expect("Response should deserialize");
...
fn test_daemon_direct_mode_consistency() {
    let helper = DaemonTestHelper::new("consistency_test").expect("Failed to create test helper");

    // Test both eol modes
    for eol in [false, true] {
        let mut aka = helper.create_aka_instance(eol).expect("Failed to create AKA instance");
...
fn test_socket_path_determination() {
    let helper = DaemonTestHelper::new("socket_test").expect("Failed to create test helper");

    let socket_path = determine_socket_path(&helper.home_dir).expect("Failed to determine socket path");

    assert!(
        socket_path.to_string_lossy().contains("aka"),
        "Socket path should contain 'aka': {:?}",
        socket_path
    );
    assert!(
        socket_path.to_string_lossy().contains("daemon.sock"),
    ...
    assert!(
        is_under_home || is_under_runtime,
        "Socket path should be under home directory or XDG_RUNTIME_DIR: {:?}",
        socket_path
    );
}
...
fn test_config_reload_integration() {
    let helper = DaemonTestHelper::new("reload_integration_test").expect("Failed to create test helper");

    // Create initial AKA instance
    let mut aka = helper
        .create_aka_instance(false)
        .expect("Failed to create AKA instance");

    // Test initial config
    let initial_result = aka
        .replace_with_mode("reload-test", ProcessingMode::Direct)
        .expect("Initial query should work");
    assert!(
        initial_result.contains("Original config"),
        "Should contain original config value: {}",
        initial_result
    );
...
"#;
    ...
    helper.update_config(updated_config).expect("Failed to update config");

    // Create new AKA instance to pick up changes
    let mut updated_aka = helper
        .create_aka_instance(false)
        .expect("Failed to create updated AKA instance");

    // Test updated config
    let updated_result = updated_aka
        .replace_with_mode("reload-test", ProcessingMode::Direct)
        .expect("Updated query should work");
    assert!(
        updated_result.contains("Updated config"),
        "Should contain updated config value: {}",
        updated_result
    );

    // Test new alias
    let new_alias_result = updated_aka
        .replace_with_mode("new-alias", ProcessingMode::Direct)
        .expect("New alias query should work");
    assert!(
        new_alias_result.contains("New alias added"),
        "Should contain new alias value: {}",
        new_alias_result
    );
}
...
fn test_daemon_mode_error_handling() {
    let helper = DaemonTestHelper::new("error_handling_test").expect("Failed to create test helper");
    ...
    let result = aka
        .replace_with_mode("non-existent-alias", ProcessingMode::Direct)
        .expect("Should handle non-existent alias");
    // The behavior might be to return empty string or original command, both are valid
    assert!(
        result.trim() == "non-existent-alias" || result.trim() == "",
        "Should return original command or empty string for non-existent alias, got: '{}'",
        result.trim()
    );

    // Test daemon mode with same non-existent alias
    let daemon_result = aka
        .replace_with_mode("non-existent-alias", ProcessingMode::Daemon)
...
fn test_performance_characteristics() {
    let helper = DaemonTestHelper::new("performance_test").expect("Failed to create test helper");
    ...
    for _i in 0..10 {
        let _result = aka
            .replace_with_mode("test-alias", ProcessingMode::Direct)
            .expect("Direct mode should work");
    }
    ...
    assert!(
        direct_elapsed < Duration::from_millis(1000),
        "Direct mode 10 operations took {}ms, should be under 1000ms",
        direct_elapsed.as_millis()
    );
    assert!(
        daemon_elapsed < Duration::from_millis(1000),
...
fn test_eol_parameter_consistency() {
    let helper = DaemonTestHelper::new("eol_test").expect("Failed to create test helper");

    // Test eol=false
    let mut aka_no_eol = helper
        .create_aka_instance(false)
        .expect("Failed to create AKA instance with eol=false");

    let result_no_eol = aka_no_eol
        .replace_with_mode("test-alias", ProcessingMode::Direct)
    ...
    assert!(
        !result_no_eol.is_empty(),
        "Result should not be empty when alias exists"
    );

    // Test eol=true
    let mut aka_with_eol = helper
        .create_aka_instance(true)
        .expect("Failed to create AKA instance with eol=true");

    let result_with_eol = aka_with_eol
        .replace_with_mode("test-alias", ProcessingMode::Direct)
...
fn test_daemon_protocol_structure() {
    ...
    assert!(
        serialized.contains("\"type\":\"Health\""),
        "Health should have type field"
    );
    ...
    assert!(
        serialized.contains("\"type\":\"Success\""),
        "Success should have type field"
    );
    assert!(
        serialized.contains("\"data\":\"test\""),
